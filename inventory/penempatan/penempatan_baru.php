...
if(isset($_POST['btnSimpan'])){
	...
	if (count($pesanError)>=1 ){
		...
		$mySql	= "INSERT INTO penempatan (no_penempatan, tgl_penempatan, kd_lokasi, keterangan, jenis, kd_petugas,pegawai)
					VALUES ('$kodeBaru', '$txtTanggal', '$cmbLokasi', '$txtKeterangan', 'Baru', '$userLogin', '$cmbpegawai')";
		...
		if($myQry){
			...
			while($tmpData = mysql_fetch_array($tmpQry)) {
				...
				$mySql = "UPDATE barang_inventaris SET status_barang='Ditempatkan', pegawai_yang_menggunakan ='$cmbpegawai' WHERE kd_inventaris='$dataKode'";
...
<table width="900" cellpadding="3" cellspacing="1" class="table-list">
    ...
    <tr>
	  <td><strong>Pegawai Yang Menggunakan </strong></td>
	  <td><strong>:</strong></td>
	  ...
	  $comboQry = mysql_query($comboSql, $koneksidb) or die ("Gagal Query".mysql_error());
	  while ($comboData = mysql_fetch_array($comboQry)) {
		if ($comboData['kd_pegawai'] == $dataDepartemen) {
			$cek = " selected";
		} else { $cek=""; }
