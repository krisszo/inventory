...
if(isset($_POST['btnSimpan'])){
	...
	if (trim($cmbDepartemen)=="Kosong") {
		$pesanError[] = "Data <b> Nama Departemen</b> belum dipilih, silahkan pilih pada combo !";		
	}
	...
	if (count($pesanError)>=1 ){
		...
		$mySql	= "INSERT INTO pengadaan (no_pengadaan, tgl_pengadaan, keterangan, kd_supplier, kd_departemen, jenis_pengadaan, kd_petugas) 
					VALUES ('$kodeBaru', '$txtTanggal', '$txtKeterangan', '$cmbSupplier', '$cmbDepartemen', '$cmbJenis', '$userLogin')";
		...
		if($myQry){
			...
			while ($tmpData = mysql_fetch_array($tmpQry)) {
				...
				for($i =1; $i <= $dataJumlah; $i++) {
					...
					$th_datang	= date('Y');
					$mySql	= "INSERT INTO barang_inventaris(kd_inventaris, kd_barang, no_pengadaan, tgl_masuk, tahun_datang, harga_barang, keterangan, status_barang) 
...
<table width="900" cellspacing="1" class="table-list" style="margin-top:0px;">
    ...
    <tr>
	  ...
	  while ($myData = mysql_fetch_array($myQry)) {
	  	...
	  	echo "<option value='$myData[kd_departemen]' $cek>[ $myData[kd_departemen] ] $myData[nm_departemen]</option>";
	  }
	  ?>
        </select>
	  </b></td>
